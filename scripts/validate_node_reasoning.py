#!/usr/bin/env python3
"""Validate the focused key-node reasoning regression suite."""
...
CASES_PATH = ROOT / "evals/node-validation-regression-cases.json"
...
def main() -> int:
    ...
    if data.get("version") != 1:
        fail("node-validation-regression-cases.json version must be 1")

    purpose = data.get("purpose")
    if not isinstance(purpose, str) or not purpose.strip():
        fail("node-validation-regression-cases.json requires purpose")

    rubric = data.get("rubric")
    if not isinstance(rubric, str) or not (ROOT / rubric).is_file():
        fail("node-validation-regression-cases.json rubric must reference an existing file")
    ...
    if len(invariants) < 6:
        fail("global_invariants must contain at least six items")

    cases = data.get("cases")
    if not isinstance(cases, list) or len(cases) < 5:
        fail("node-validation-regression-cases.json must contain at least five cases")

    ids: set[str] = set()
    for case in cases:
        if not isinstance(case, dict):
            fail("Each node-validation case must be an object")

        case_id = case.get("id")
        if not isinstance(case_id, str) or not re.fullmatch(r"[a-z0-9][a-z0-9-]*", case_id):
            fail(f"Invalid node-validation case id: {case_id!r}")
        if case_id in ids:
            fail(f"Duplicate node-validation case id: {case_id}")
        ids.add(case_id)
    ...
    if missing:
        fail("Missing required node-validation cases: " + ", ".join(missing))

    print(
        "Validated node reasoning suite: "
