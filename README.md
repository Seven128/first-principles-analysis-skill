...
- “最终目的”原则上只写一句，不附加限制、取舍或方案。
- “现状与要解决的问题”始终比较同一主体、A0 与 O，只承载最少充分事实、顶层能力缺口、固有限制和根问题。
- 具体任务的完整动态过程、方案加入后的新基线、实现机制与运行流程必须在后文逐项推出。
- 内部推演脚手架不应泄漏到面向读者的正文。
...
- 能否选择正确时间基线；
- 最终目的是否原则上只有一句；
- 现状部分是否始终比较同一主体、固定 A0 与最终目的 O；
- 现状是否停留在顶层能力缺口、固有限制和根问题，而不是具体任务过程或未来实现；
- 是否在方案部分逐项更新基线并正向推出机制；
- 是否保持主体与抽象层级一致；
- 是否只在必要时讨论版本差异；
- 是否删除对读者无价值的内部脚手架；
- 事实、推断和待验证项是否分开。
