...
- Use information-bearing headings that state the subject and relationship. Add subheadings for independent semantic units; compact aspects may use ordinary prose or optional emphasis of a complete conclusion. Do not automatically bold every paragraph's first sentence or pre-explain every child section. Several paragraphs supporting one argument do not automatically require separate headings or repeated conclusions.
...
- For purpose-bearing problems, preserve the semantic order: final purpose or target state → current facts, capabilities, and constraints → missing conditions and problems → design principles → concrete paths → completion criteria, validation, cost, and boundaries.
- Keep the final purpose at the semantic level appropriate to the problem: an external outcome for goal solving, or a stable role / target state for designed-system explanation. Keep completion criteria and modules, protocols, algorithms, class names, or implementation properties in their later nodes.
